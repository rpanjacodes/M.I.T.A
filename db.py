...
async def init_db():
    async with pool.acquire() as conn:
        await conn.execute('''
            CREATE TABLE IF NOTimport asyncpg
import asyncio
    ...
    async with pool.acquire() as conn:
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS settings (
                guild_id BIGINT PRIMAimport asyncpg
import asyncio
...
async def set_last_video_id(guild_id, platform, post_id):
    try:
        ...
        print(f"[DB] set_last_video_id error: {e}")RY KEY,
                nickname_toggle INTEGER DEFAULT 0,
                nickname_format TEXT DEFAULT 'User_{username}'
            );
        ''')
...
async def set_last_video_id(guild_id, platform, post_id):
    try:
        ...
        print(f"[DB] set_last_video_id error: {e}") EXISTS settings (
                guild_id BIGINT PRIMAimport asyncpg
import asyncio
